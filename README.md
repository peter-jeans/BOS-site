...
- Generate the user-install capability catalogue from `OS/INSTALLERS/USER_INSTALL_CAPABILITY_CATALOGUE.json`, resolved against the executable registry and maturity signals. Publish only its explicit allowlist, including public-safe Experimental capability labels, access route, limitations and consent notice. Never publish raw Building Zone programmes, authority paths, component evidence, readiness/risk/approval internals, credentials, source, raw prompts, raw receipts, DevPacks, or `.bos` state.
